...
**Components with Host Backend integration:**

![Components](./assets/components.png)

**Components in a Browser Standalone Setup:**

![Components Browser Standalone](./assets/components-browser-standalone.png)

!!! note
    With this setup all components (Microfrontend Server, APIs, ...) must be publicly accessible and CORS must be enabled.
    Furthermore, some API security measures (e.g., BASIC), user permission and server-side rendering are not available.
...
- **Backend API:** The API a *Microfrontend* uses to fetch data or perform actions. This can be a REST API, GraphQL, or any other type of API that the *Microfrontend*770 needs to function correctly.
